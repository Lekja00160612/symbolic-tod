...
def f1_score_state(predict, label):
    label_state, label_history, label_nextacts = label.split("\t")

    predict_outputs = predict.split("\t")
    if len(predict_outputs) == 3:
        predict_state, predict_history, predict_nextacts = predict_outputs
